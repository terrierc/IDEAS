...
# ─── MY PORTFOLIO ────────────────────────────────────────────────────────────
    ...
    if not trades:
        ...
        holdings = get_holdings()
        trades = get_all_trades()
        st.info("Sample ASX portfolio loaded automatically. Replace with your own trades in the **Add Trade** tab.")
...
# ─── BENCHMARKING ────────────────────────────────────────────────────────────
    ...
    if trades and summary.get("inception_date"):
        st.divider()
        ...
        if st.button("Run Comparison", type="primary"):
            with st.spinner("Fetching market data..."):
                try:
                    ...
                    bench_tickers = [v for v in list(BENCHMARKS.values())[:3]]
...
# ─── THE PITCH ───────────────────────────────────────────────────────────────
    ...
    summary = portfolio_summary()
    default_return = summary.get("total_pct_return") or 12.0

    c1, c2 = st.columns(2)
    personal_return = c1.number_input("Your Portfolio Return (%)", value=float(default_return), step=0.5)
